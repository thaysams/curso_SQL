...
select
 	max((current_date - birth_date)/365) as "idade máxima",
	min((current_date - birth_date)/365) as "idade mínima"
from sales.customers
...
select brand, count (*) as quantidade
from sales.products
group by brand
order by brand
...
select brand, model_year, count(*) as "quantidade"
from sales.products
